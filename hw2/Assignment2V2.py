...
def printF(*args, **kwargs):
    pickle.dump("\n", infile)
    pickle.dump(" ".join(map(str, args)), infile)
    print(" ".join(map(str, args)))

#main
if __name__ == '__main__':
    infile = open(b"resultsA.txt", "wb") #opens a txt file
    twitter_api = oauth_login()
#get initial start time to see how long the script ran for at the end
    startTime = datetime.datetime.now()
    printF("Start Time:",str(startTime) )

    screenName = "edmundyu1001" #sets twitter user as the root node
...
#get the time when program is done
    ...
    printF("Total time: ", str(finishTime - startTime) )
